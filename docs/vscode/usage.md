# Usage of Reicon VS Code Extension
...
Open the Extensions panel in VS Code (Cmd+Shift+X or Ctrl+Shift+X), search for **Reicon**, and click install. Or install it via command-line interface:

```bash
code --install-extension DevChauhan.reicon
```
...
1. **Open the Sidebar Explorer**: Click on the **Reicon** logo in the VS Code Activity Bar (located on the left-side toolbar).
