import createIcon from '../createIcon.js';

/**
 * @component
 * @name Book4
 * @description Reicon Vue icon component, renders an SVG Element.
 * @preview ![Book4](data:image/svg+xml;base64,) - https://reicon.dev/icons/book4
 * @see https://reicon.dev/docs — Documentation
 */
const Book4 = createIcon('Book4', {
  O: `<path fill-rule="evenodd" clip-rule="evenodd" d="M2.25 19.5V5.5C2.25 3.15279 4.15279 1.25 6.5 1.25H19.5C20.7426 1.25 21.75 2.25736 21.75 3.5V15.5C21.75 16.6581 20.875 17.6119 19.75 17.7363V21.25H21C21.4142 21.25 21.75 21.5858 21.75 22C21.75 22.4142 21.4142 22.75 21 22.75H5.5C3.70507 22.75 2.25 21.2949 2.25 19.5ZM3.75 5.5C3.75 3.98122 4.98122 2.75 6.5 2.75H19.5C19.9142 2.75 20.25 3.08579 20.25 3.5V15.5C20.25 15.9142 19.9142 16.25 19.5 16.25H5.5C4.85568 16.25 4.25515 16.4375 3.75 16.7609V5.5ZM18.25 17.75H5.5C4.5335 17.75 3.75 18.5335 3.75 19.5C3.75 20.4665 4.5335 21.25 5.5 21.25H18.25V17.75Z" fill="currentColor"/>`,
  F: `<path fill-rule="evenodd" clip-rule="evenodd" d="M2.25 5.5V19.5C2.25 21.2949 3.70507 22.75 5.5 22.75H21C21.4142 22.75 21.75 22.4142 21.75 22C21.75 21.5858 21.4142 21.25 21 21.25H19.75V17.7363C20.875 17.6119 21.75 16.6581 21.75 15.5V3.5C21.75 2.25736 20.7426 1.25 19.5 1.25H6.5C4.15279 1.25 2.25 3.15279 2.25 5.5ZM18.25 17.75H5.5C4.5335 17.75 3.75 18.5335 3.75 19.5C3.75 20.4665 4.5335 21.25 5.5 21.25H18.25V17.75Z" fill="currentColor"/>`
});

export { Book4 };
export default Book4;
