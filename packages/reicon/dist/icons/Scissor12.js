import createIcon from '../createIcon.js';

/**
 * @name Scissor12
 * @description Reicon SVG icon function, creates an SVG element.
 * @preview ![Scissor12](data:image/svg+xml;base64,) - https://reicon.dev/icons/scissor-12
 * @see https://reicon.dev/docs — Documentation
 * @param {import('../createIcon').IconOptions} [options] — Icon options
 * @returns {SVGSVGElement} SVG Element
 */
const Scissor12 = createIcon('Scissor12', {
  O: `<path d="M5.5 10C7.433 10 9 8.433 9 6.5C9 4.567 7.433 3 5.5 3C3.567 3 2 4.567 2 6.5C2 8.433 3.567 10 5.5 10Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M5.5 21C7.433 21 9 19.433 9 17.5C9 15.567 7.433 14 5.5 14C3.567 14 2 15.567 2 17.5C2 19.433 3.567 21 5.5 21Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M22 6L8.64999 15.98" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M22 17.97L8.64999 7.97998" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>`,
  F: `<path d="M5.5 10C7.433 10 9 8.433 9 6.5C9 4.567 7.433 3 5.5 3C3.567 3 2 4.567 2 6.5C2 8.433 3.567 10 5.5 10Z" fill="currentColor"/><path d="M5.5 21C7.433 21 9 19.433 9 17.5C9 15.567 7.433 14 5.5 14C3.567 14 2 15.567 2 17.5C2 19.433 3.567 21 5.5 21Z" fill="currentColor"/><path d="M15.2491 11.9803L22.4491 6.60027C22.7791 6.35027 22.8491 5.88027 22.5991 5.55027C22.3491 5.22027 21.8791 5.15027 21.5491 5.40027L13.9991 11.0503L8.4491 6.89027C8.1191 6.65027 7.6491 6.71027 7.3991 7.04027C7.1491 7.37027 7.2191 7.84027 7.5491 8.09027L12.7491 11.9803L6.8491 16.4003C6.5191 16.6503 6.4491 17.1203 6.6991 17.4503C6.8491 17.6503 7.0691 17.7503 7.2991 17.7503C7.4591 17.7503 7.6091 17.7003 7.7491 17.6003L13.9991 12.9203L21.5491 18.5703C21.6791 18.6703 21.8391 18.7203 21.9991 18.7203C22.2291 18.7203 22.4491 18.6203 22.5991 18.4203C22.8491 18.0903 22.7791 17.6203 22.4491 17.3703L15.2491 11.9803Z" fill="currentColor"/>`
});

export { Scissor12 };
export default Scissor12;
